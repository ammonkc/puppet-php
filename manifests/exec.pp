...
define php::exec (
  ...
  Exec { path => "/bin:/usr/bin/:/sbin:/usr/sbin:${php::composer::target_dir}" }
  ...
  $command = "php ${php::composer::target_dir}/${php::composer::composer_file} ${cmd}"

  exec { "composer_update_${title}":
    command   => template('php/exec.erb'),
    cwd       => $cwd,
    logoutput => $logoutput,
  }
}
